...
//! Project version number for CCWeakTimer.
FOUNDATION_EXPORT double CCWeakTimerVersionNumber;
 ...
 */
@interface CCWeakTimer : NSObject
 ...
 * @param repeats if `YES`, `selector` will be invoked on `target` until the `CCWeakTimer` object is deallocated or until you call `invalidate`. If `NO`, it will only be invoked once.
...
/**
 * Creates an `CCWeakTimer` object and schedules it to start ticking inmediately.
 */
 ...
 * @note `CCWeakTimer` won't invoke the `selector` on `target` again after calling this method.
